...
/**
 * Discreet top-nav control for program admins only: toggles between Protected
 * (default) and Normal presentation. Hidden for tower-lead-only sessions.
 * Icon-only by design — the active state is communicated through colour and a
 * subtle indicator dot, not a visible label, so a client looking at the screen
 * has no obvious cue that an alternate view exists. The accessible label is
 * delivered through the `title` and `aria-pressed` attributes.
 *
...
export function ClientModeToggle() {
  const { clientMode, mounted, toggleClientMode, canToggleClientMode } = useClientMode();
  if (!canToggleClientMode) return null;
