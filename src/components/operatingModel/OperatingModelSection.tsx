...
export function OperatingModelSection({
  ...
  if (IS_V6) {
    return <OperatingModelSectionV6 tower={tower} showRoadmap={showRoadmap} />;
  }
  return <OperatingModelSectionV5 tower={tower} showRoadmap={showRoadmap} />;
}

function OperatingModelSectionV5({
  tower,
  showRoadmap = true,
}: {
  tower: Tower;
...
// ===========================================================================
//   v6 — L3 Job Family grain
// ===========================================================================

/**
 * v6 sibling of `OperatingModelSectionV5`. Loops `l3Rows` (the
 * dial-bearing row) and renders one card per L3 Job Family with 1..N
 * specific AI Solutions hanging off it (no L4 Activity Group / L5
 * Activity tree). Cross-tower priority still lives on the Cross-Tower
 * AI Plan page; the per-tower view shows the binary feasibility signal
 * via `AiRoadmapV6`.
 */
function OperatingModelSectionV6({
  tower,
  showRoadmap = true,
}: {
  tower: Tower;
...
/**
 * v6 empty-state pane — same three legitimate "empty" scenarios as the
 * v5 helper, retold in L3-grain language.
 */
