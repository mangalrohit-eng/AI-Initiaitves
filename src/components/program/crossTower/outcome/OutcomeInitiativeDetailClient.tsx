...
/**
 * Cross-Tower Discrete Initiative — brief detail page.
 *
 * Route: `/program/cross-tower-ai-plan/outcome/[clusterId]/initiative/[initiativeId]`.
 *
 * Sections (mirrors the tower-specific Solution Brief depth):
 *   1. Header — name, tier, modeled $ rollup, cluster crumb, towers
 *   2. Current → Future state
 *   3. Constituent AI Solutions
 *   4. Agent architecture (union across anchored solutions)
 *   5. Sourcing mix (Build / Buy / Discover)
 *   6. Dependencies (strategist-authored + digital-core platforms)
 *   7. Reference architecture (workbench + digital-core digest)
 */
...
import {
  ...
  aggregateVendors,
  anchoredSolutionsForInitiative,
  deriveValueTier,
  initiativeRollupUsd,
...
export function OutcomeInitiativeDetailClient({
  clusterId,
  initiativeId,
}: {
  clusterId: string;
  ...
  const initiative: StrategistInitiative | null = React.useMemo(() => {
    if (!strategistOutputs) return null;
    return (
      strategistOutputs.initiatives.find(
        (i) => i.id === initiativeId && i.clusterId === clusterId,
      ) ?? null
    );
  }, [strategistOutputs, clusterId, initiativeId]);
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <Breadcrumbs
          items={[
            ...
            cluster
              ? {
                  label: cluster.title,
                  href: `/program/cross-tower-ai-plan/outcome/${cluster.id}`,
                }
        ...
        <div className="mt-4">
          <Link
            href={
              cluster
                ? `/program/cross-tower-ai-plan/outcome/${cluster.id}`
          ...
          >
            ...
            {cluster ? "Back to outcome cluster" : "Back to Cross-Tower AI Plan"}
          </Link>
        </div>
        ...
        {!strategistOutputs ? (
          ...
          <InitiativeNotFound initiativeId={initiativeId} />
        ) : (
          <InitiativeBriefBody
            cluster={cluster}
...
function InitiativeBriefBody({
  ...
  const processes = anchored.map((p) => processByInitiativeId.get(p.id));
  const vendors = aggregateVendors(anchored);
  ...
  return (
    <div className="mt-6 space-y-8">
      ...
      <Section
        id="current-future"
        eyebrow="Current → Future"
        title="What changes before and after"
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border border-forge-border bg-forge-well/30 p-3">
            ...
            <p className="mt-2 text-[12.5px] leading-relaxed text-forge-body">
          ...
          <div className="rounded-lg border border-accent-purple/30 bg-accent-purple/5 p-3">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-wider text-accent-purple-dark">
              <span>&gt;</span> With this initiative live
            </p>
            <p className="mt-2 text-[12.5px] leading-relaxed text-forge-body">
      ...
      <Section
        id="constituent-solutions"
        eyebrow="Constituent AI Solutions"
        title={`${anchored.length} tower-specific solution${anchored.length === 1 ? "" : "s"} power this initiative`}
      >
        {anchored.length === 0 ? (
          <UnsizedHint scope="initiative" />
        ) : (
      ...
      >
        {agents.length === 0 ? (
          <EmptySectionHint message="No agent details cached on the anchored solutions yet. Open each tower brief to generate." />
        ) : (
      ...
      <Section
        ...
        title="Data, systems, and prior work this initiative depends on"
      >
        {initiative.dependencies.length === 0 ? (
          <EmptySectionHint message="The strategist did not surface explicit dependencies for this initiative." />
        ) : (
          <ul className="list-disc space-y-1.5 pl-5 text-[12.5px] leading-relaxed text-forge-body">
      ...
      <Section
        ...
        title="Workbench + digital-core digest from anchored briefs"
      >
        {lensDigest.digitalCorePlatforms.length === 0 &&
        lensDigest.workbenchTools.length === 0 &&
        lensDigest.workforceChanges.length === 0 ? (
          <EmptySectionHint message="No four-lens briefs have been generated on the anchored solutions yet. Open each tower brief to populate." />
        ) : (
          <div className="space-y-4">
            {lensDigest.digitalCorePlatforms.length > 0 ? (
              <div>
                <p className="font-mono text-[10px] font-semibold uppercase tracking-wider text-forge-subtle">
                  ...
                  Required digital-core platforms
                </p>
                <ChipCloud
            ...
            {lensDigest.workbenchTools.length > 0 ? (
              <div>
                <p className="font-mono text-[10px] font-semibold uppercase tracking-wider text-forge-subtle">
                  <span className="text-accent-purple-dark">&gt;</span>{" "}
                  Workbench tools the future state lands on
                </p>
                <ChipCloud labels={lensDigest.workbenchTools} tone="teal" />
              </div>
            ) : null}
            {lensDigest.workforceChanges.length > 0 ? (
              <div>
    ...
    <header className="mt-2 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-start">
      <div className="max-w-3xl">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-accent-purple/30 bg-accent-purple/5 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-accent-purple-dark">
          ...
          <Link
            href={`/program/cross-tower-ai-plan/outcome/${cluster.id}`}
            className="underline decoration-accent-purple/40 underline-offset-2 hover:decoration-accent-purple/80"
          >
            {cluster.title}
          </Link>
        </span>
        ...
        <TowerChipRow towers={initiative.towers} />
      </div>
      <div className="flex flex-col items-stretch gap-3 lg:items-end">
        <ValueTierBadge tier={tier} />
        <RollupCard
          label="Modeled $ for this initiative"
          value={dollarLabel}
          footnote={
            anchoredCount === 0
...
// ---------------------------------------------------------------------
//   Agent + sourcing renderers (same shapes as cluster client)
// ---------------------------------------------------------------------
        ...
        <div className="mt-4">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {vendors.map((v) => (
              ...
              >
                <ShieldCheck
                  className="h-3 w-3 text-accent-purple-dark"
                  aria-hidden
                />
                {v.vendor}
                {v.count > 1 ? (
  ...
  return (
    <div className="mt-10 rounded-2xl border border-dashed border-forge-border bg-forge-surface p-8 text-center">
      ...
      <p className="mx-auto mt-2 max-w-xl text-[13px] leading-relaxed text-forge-body">
        Open the Cross-Tower AI Plan and generate strategist outputs.
        Once they exist, this initiative brief will render with rolled-up
        modeled $, agent footprint, and sourcing mix from the anchored
        AI Solutions.
      </p>
      <Link
        href="/program/cross-tower-ai-plan"
...
function InitiativeNotFound({ initiativeId }: { initiativeId: string }) {
  return (
    <div className="mt-10 rounded-2xl border border-dashed border-forge-border bg-forge-surface p-8 text-center">
      <Layers className="mx-auto h-7 w-7 text-forge-subtle" aria-hidden />
      <h2 className="mt-3 font-display text-lg font-semibold text-forge-ink">
        Initiative not found in this cluster
      </h2>
      <p className="mx-auto mt-2 max-w-xl text-[13px] leading-relaxed text-forge-body">
        The initiative id <code className="font-mono">{initiativeId}</code> is
        not present in the most recent strategist run for this cluster.
        Regenerate the strategist or return to the plan.
      </p>
      <Link
        href="/program/cross-tower-ai-plan"
        className="mt-4 inline-flex items-center gap-2 rounded-lg bg-accent-purple px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-accent-purple-dark"
