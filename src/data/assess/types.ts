...
/**
 * @deprecated Pre-v6 alias of `L4WorkforceRow` (the 5-layer migration kept
 * this name briefly so files written against the older `L3WorkforceRow`
 * type still compiled). Retained ONLY as a transitional alias under v6 —
 * v6 adds a NEW `L3WorkforceRowV6` (see below) that is the genuine
 * Job-Family-grain primary entity. Once Phase 7 cleanup ships, this alias
 * is deleted; v5-only callers must rename to `L4WorkforceRow` first.
 */
export type L3WorkforceRow = L4WorkforceRow;

// ============================================================================
//   V6 ADDITIONS — L3-grain initiatives cutover (Phase 1)
// ============================================================================

/**
 * One AI Solution product generated by the per-L3 LLM curation pipeline.
 * 1-N of these attach to each `L3WorkforceRowV6` (typically 1-3).
 *
 * Solution-altitude product, not an activity name. The LLM curation prompt
 * forces the `solutionName` field to follow the
 * "Agentic AI <domain> <Solution|Co-Pilot|Assistant>" pattern
 * (e.g. "Agentic AI Financial Close Co-Pilot", "Agentic AI News Production
 * Co-Pilot") and reject any name that echoes the underlying L4 / L5
 * activity vocabulary. The full 4-lens design (work / workforce / workbench
 * / digitalCore / agents / agentOrchestration) is inlined as a `Process`
 * object so click-through to the deep-dive page needs no separate join.
 */
export type L3Initiative = {
  ...
  tagline: string;
  /** Versant-grounded rationale (2-4 sentences). Names brands, vendors, JV structures. */
  aiRationale: string;
  /** Binary ship-readiness signal — feeds the cross-tower 2x2. */
  feasibility: Feasibility;
   ...
   * the deep-dive route on first click — same pattern as v5's
   * `L5Item.generatedProcess`. Absent on freshly curated initiatives so
   * the per-L3 curation call returns in seconds, not minutes.
   */
  generatedProcess?: GeneratedProcessCache;
  /** Provenance. */
...
/**
 * One L3 Job Family row in the v6 tower workforce footprint. Replaces the
 * v5 `L4WorkforceRow` as the dial-bearing primary entity for Step 2 and
 * the curation grain for Step 4.
 *
 * Headcount + spend are aggregated sums of the child L4 rows; the L4 rows
 * themselves remain in storage (untouched on `TowerAssessState.l4Rows`)
 * as read-only LLM context — the LLM curation prompt receives the full
 * "what's inside this L3" detail (child L4 names + L5 candidate names)
 * but the dial and the AI Solution attach to L3.
 *
 * Derived from `l4Rows` on Step 1 upload via `deriveL3Rows()` (groups by
 * `(l1, l2, l3)`, sums headcount + spend). The v5 -> v6 read-time
 * migration runs the same derivation once and marks every row
 * `curationStage: "queued"` so the StaleCurationBanner invites the user
 * to re-run curation (per the "fresh-start with backup" migration
 * strategy locked in by the plan).
 *
 * Field name suffix `V6` distinguishes this from the deprecated v5 alias
 * `L3WorkforceRow` (= `L4WorkforceRow`). Phase 7 cleanup deletes the alias
 * and renames this back to `L3WorkforceRow` once v5 is dead.
 */
export type L3WorkforceRowV6 = {
  /** Stable id — slugified `${towerId}::${l1}::${l2}::${l3}`. */
  id: string;
  /** L1 Function (= the tower's L1 function name from `towerFunctionNames.ts`). */
  l1: string;
  /** L2 Job Grouping. */
  l2: string;
  /** L3 Job Family — the dial-bearing layer under v6. */
  l3: string;
  /** Sum of child L4 fteOnshore. */
  fteOnshore: number;
  /** Sum of child L4 fteOffshore. */
  fteOffshore: number;
  /** Sum of child L4 contractorOnshore. */
  contractorOnshore: number;
  /** Sum of child L4 contractorOffshore. */
  contractorOffshore: number;
  /** Sum of child L4 `annualSpendUsd` (only present when at least one child has it). */
  annualSpendUsd?: number;
  ...
  /** LLM-produced rationale for the offshore dial (<=15 words, Versant-grounded). */
  offshoreRationale?: string;
  /** LLM-produced rationale for the AI dial (<=15 words, Versant-grounded). */
  aiImpactRationale?: string;
  /** Provenance for the dial rationale pair (drives the chip next to each slider). */
  dialsRationaleSource?: "llm" | "heuristic" | "starter";
  /** ISO timestamp the dial rationale pair was last written. */
  dialsRationaleAt?: string;
  /** Back-pointers to the underlying L4 context rows on `TowerAssessState.l4Rows`. */
  childL4RowIds: string[];
  /** AI Solution products generated by the per-L3 LLM curation pipeline. */
  l3Initiatives?: L3Initiative[];
  /**
   * Stable hash of the L3's identity + child L4 footprint + dial values.
   * The pipeline's idempotency key — re-runs skip rows whose hash matches
   * the last successful run. Distinct from per-initiative `L3Initiative.id`.
   */
  curationContentHash?: string;
  /** Whole-row pipeline status. */
  curationStage?: CurationStage;
...
export type TowerAssessState = {
  ...
  /**
   * V6 ADDITION — derived L3 Job Family rows. The dial-bearing primary
   * entity under v6 (Step 2 dials live here, AI Solutions attach here).
   *
   * Optional for back-compat: v5 snapshots and v5-mode reads leave this
   * undefined and the v5 selectors keep working off `l4Rows`. The v5 ->
   * v6 read-time migration in `assessProgramIO.ts` derives this once
   * from `l4Rows` (groups by `(l1, l2, l3)`, sums headcount + spend, marks
   * each row `curationStage: "queued"`).
   *
   * Both arrays coexist on the same TowerAssessState — `l4Rows` is kept as
   * read-only context for the LLM prompt; `l3Rows` is what Step 2 / Step 4
   * read and write.
   */
  l3Rows?: L3WorkforceRowV6[];
 ...
 *   the dial + initiative grain up one level to L3 Job Family.
 */
...
/** Back-compat alias — every caller importing `AssessProgramV5` gets V6. */
export type AssessProgramV5 = AssessProgramV6;

/** Back-compat alias — every caller importing `AssessProgramV4` gets V6. */
export type AssessProgramV4 = AssessProgramV6;

/** Back-compat alias — every caller importing `AssessProgramV3` gets V6. */
export type AssessProgramV3 = AssessProgramV6;

/** Back-compat alias — every caller importing `AssessProgramV2` gets V6. */
export type AssessProgramV2 = AssessProgramV6;
