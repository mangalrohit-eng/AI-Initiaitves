...
const finProcurement = processShell("fin-5", "Procurement Optimization & Vendor Management", {
  ...
  work: {
    ...
    post: workState(
      ...
      [
        ["Normalize spend signals across entities", "Spend Analytics Agent", "2 hours", false],
        ["Match demand to preferred vendors and tiers", "Vendor Matching Agent", "3 hours", false],
        ["Clause risk + savings insights", "Contract Intelligence Agent", "4 hours", false],
        ["Touchless invoice validation and exceptions", "Invoice Processing Agent", "2 hours", false],
        ["Vendor risk posture refresh", "Vendor Risk Agent", "continuous", false],
      ],
  ...
  agents: [
    agent(
      ...
      "Spend Analytics Agent",
      "Classifies demand, harmonizes SKUs, and surfaces category leakage",
      "Specialist",
      ["intake forms", "SKUs", "historical POs"],
      ["spend cubes", "category insights"],
      true,
      ["P2P API", "LLM", "data warehouse"],
    ),
    agent(
      "vendor-matching-agent",
      "Vendor Matching Agent",
      "Scores suppliers against policy tiers, diversity goals, and performance history",
      "Router",
      ["scoped need", "shortlists"],
      ["ranked vendors", "award paths"],
      true,
      ["LLM", "supplier graph", "rules engine"],
    ),
    agent(
      "contract-intelligence-agent",
      "Contract Intelligence Agent",
      "Flags non-standard clauses, renewal windows, and savings opportunities",
      "Specialist",
      ["contracts", "benchmarks"],
      ["risk report", "redlines"],
      true,
      ["CLM API", "LLM"],
    ),
    agent(
      ...
      "Invoice Processing Agent",
      "Validates invoices, three-way match, and routes exceptions",
      "Executor",
      ["POs", "receipts", "invoices"],
      ["posting packets", "exception queue"],
      false,
      ["P2P", "OCR", "ERP"],
    ),
    agent(
      "vendor-risk-agent",
      "Vendor Risk Agent",
      "Monitors financial health, cyber posture, and geopolitical exposure for critical suppliers",
      "Monitor",
      ["external feeds", "contracts"],
      ["risk scores", "alerts"],
      false,
      ["D&B", "security ratings", "RSS"],
    ),
  ],
  agentOrchestration: orchestration(
    "Parallel",
    "Spend, matching, contract intelligence, invoice automation, and vendor risk run concurrently before human award decisions.",
    [
      { from: "spend-analytics-agent", to: "vendor-matching-agent", dataPassed: "category signals", trigger: "Intake" },
      { from: "spend-analytics-agent", to: "contract-intelligence-agent", dataPassed: "SKU + vendor context", trigger: "Draft MSA" },
      { from: "vendor-matching-agent", to: "invoice-processing-agent", dataPassed: "selected vendor", trigger: "Award" },
      { from: "contract-intelligence-agent", to: "vendor-risk-agent", dataPassed: "clause flags", trigger: "High spend" },
    ],
  ),
});
