"use client";

import * as XLSX from "xlsx";
import { getTowerSeedRows } from "@/data/assess/seedAssessProgram";
import type { L3WorkforceRow, TowerId } from "@/data/assess/types";
import { towers } from "@/data/towers";

/**
 * Tower workforce upload columns. The template is L2/L3 only — L4 activities
 * are display-only and either seeded from the canonical map or generated by
 * the LLM after upload, so they don't appear here. Impact-lever percentages
 * are also intentionally absent: tower leads dial those on the Configure
 * Impact Levers page after the headcount lands.
 */
const HEADER = [
  "L2",
  "L3",
  "FTE_onshore",
  "FTE_offshore",
  "contractor_onshore",
  "contractor_offshore",
  "annual_spend_usd",
] as const;

function rowToArray(r: L3WorkforceRow): (string | number)[] {
  return [
    r.l2,
    r.l3,
    r.fteOnshore,
    r.fteOffshore,
    r.contractorOnshore,
    r.contractorOffshore,
    r.annualSpendUsd ?? "",
  ];
}

function safeSheetName(id: string): string {
  const s = id.replace(/[:\\/?*[\]]/g, "-").slice(0, 31);
  return s || "sheet";
}

/** Multiline CSV with header for one tower (seed sample or custom rows). */
export function buildAssessTowerCsv(rows: L3WorkforceRow[]): string {
  const lines = [HEADER.join(",")];
  for (const r of rows) {
    const esc = (x: string) => (x.includes(",") || x.includes('"') ? `"${x.replace(/"/g, '""')}"` : x);
    lines.push(
      [
        esc(r.l2),
        esc(r.l3),
        r.fteOnshore,
        r.fteOffshore,
        r.contractorOnshore,
        r.contractorOffshore,
        r.annualSpendUsd != null ? r.annualSpendUsd : "",
      ].join(","),
    );
  }
  return lines.join("\n");
}

export function downloadBlob(filename: string, content: string, mime: string): void {
  const blob = new Blob([content], { type: mime });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = filename;
  a.click();
  URL.revokeObjectURL(url);
}

export function downloadSingleTowerSampleCsv(towerId: TowerId, fileBase: string): void {
  const rows = getTowerSeedRows(towerId);
  const csv = buildAssessTowerCsv(rows);
  downloadBlob(`${fileBase.replace(/[^a-z0-9-_]+/gi, "-")}-sample.csv`, csv, "text/csv;charset=utf-8");
}

/**
 * One workbook, one sheet per tower, same columns as the empty template; cells use the current seed.
 */
export function downloadAllTowersSampleWorkbook(): void {
  const wb = XLSX.utils.book_new();
  for (const t of towers) {
    const id = t.id as TowerId;
    const rows = getTowerSeedRows(id);
    const aoa: (string | number)[][] = [Array.from(HEADER), ...rows.map(rowToArray)];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    XLSX.utils.book_append_sheet(wb, ws, safeSheetName(t.id));
  }
  XLSX.writeFile(wb, "forge-tower-capability-map-samples-13-towers.xlsx");
}
