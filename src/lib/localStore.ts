...
function migrateAssessProgram(raw: unknown): AssessProgramV2 {
  ...
  for (const [k, vRaw] of Object.entries(towersRaw)) {
    ...
    // here — the v6 reads coerce through the typed selectors.
  ...
  return {
    // All persisted programs are v6. We accept legacy v5 blobs only at
    // file-import time (`assessProgramIO.importAssessProgramFromJsonText`),
    // which derives `l3Rows` and stamps `version: 6` before they reach
    // the read path.
    version: 6,
    towers,
